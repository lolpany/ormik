...
public class TableTruncater extends EnvironmentBoundAction<Void> {

    private final Connection connection;
    private final TablesToRecreateGenerator tablesToRecreateGenerator;
    private final String table;

    TableTruncater(Connection connection, TablesToRecreateGenerator tablesToRecreateGenerator, String table) {
        super(null);
    ...
    TableTruncater(int envNumber, String jdbcUrl, String password, TablesToRecreateGenerator tablesToRecreateGenerator,
    ...
    protected Void runOn(Integer envNumber) throws Exception {
        ...
        for (Pair<String, String> referencingConstraint : referencingConstraints) {
            execute(connection, new IPreparedStatementJdbcCallback<Void>() {
                @Override
                public PreparedStatement createPreparedStatement(Connection c) throws SQLException {
                    return c.prepareStatement(" alter table " + referencingConstraint.getLeft() + " disable constraint " + referencingConstraint.getRight());
                }
        ...
        for (Pair<String, String> referencingConstraint : referencingConstraints) {
            execute(connection, new IPreparedStatementJdbcCallback<Void>() {
                @Override
                public PreparedStatement createPreparedStatement(Connection c) throws SQLException {
                    return c.prepareStatement(" alter table " + referencingConstraint.getLeft() + " enable  constraint " + referencingConstraint.getRight());
                }
